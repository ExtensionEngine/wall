...
// Resource schema
internals.resource = internals.er.keys({
...
module.exports = Joi.object({
  ...
  session: Joi.object({
    name: Joi.string().optional(),
    redirect: Joi.boolean().default(true).optional(),
    loginForm: Joi.string().uri({ allowRelative: true }).required(),
    logout: Joi.object({
      method: Joi.string().valid('GET', 'POST').default('POST').optional(),
      uri: Joi.string().uri({ allowRelative: true }).required(),
      auth: Joi.string().optional(),
      finalize: Joi.func().maxArity(2).optional()
    }).required()
  }).required(),
  store: Joi.object({
    ...
    encryption: Joi.object({
      ...
      password: Joi.string().min(32).optional().when('enabled', {
        is: true,
        then: Joi.required()
      })
    }).optional(),
    userKey: Joi.string().default('sub').optional()
  }).optional()
