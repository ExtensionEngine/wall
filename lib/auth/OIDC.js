...
module.exports = class OIDC extends Entity {
  ...
  doAuthenticate(request, reply) {
    ...
    return Co(function*() {
      ...
      .then(({ credentials, token }) => {
        Wall.Instance.emit('profile', { source: this.options.name, user: credentials.user, token });
        Util.setAuthentication(request, reply, { credentials });
      })
      .catch((err) => {
        Log.error(err);
        return reply(err);
  ...
  persist(user, token) {
    return this.options.store.prepare(user, this.options.client, this.name, token).upsert();
  }
};
