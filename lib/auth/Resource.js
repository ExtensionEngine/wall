...
module.exports = class Resource extends Scheme {
  ...
  authorize(request, reply) {
    ...
    return this.token(request.query.code, {
      ...
      .then((token) => this.options.store.prepare(Util.getUser(request), this.options.client, token).upsert())
  ...
  refresh(request, reply, token) {
    if (token.expired()) {
      ...
      return Co(function*() {
        ...
        const upserted = yield store.prepare(user, client, Object.assign({}, token, refreshed)).upsert();
