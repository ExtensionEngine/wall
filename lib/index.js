...
internals.getStore = function(options) {
  ...
  if (typeof options.implementation === 'string') {
    if (options.implementation === 'mongo') {
      return new MongoStore(number, options.url, options.name);
    }
  } else if (typeof options.implementation === 'function') {
    return internals.validate(
      Store.schema,
      options.implementation(number, options.url, options.name)
    );
  }

  const store = internals.validate(Store.schema, options.implementation);
  store.number = number;
...
exports.standalone = function(options) {
  Hoek.assert(!exports.Instance, 'Already using Wall in a plugin mode');
  const cache = new Map();
  const serverMock = {
    auth: {
      default() {},
