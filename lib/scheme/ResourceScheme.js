...
internals.authenticate = function(request, reply) {
  ...
  return store.find(tid)
    .then((token) => {
      if (token) {
        return internals.setAuthorization(request, reply, session, store, user, client, token);
      }
...
internals.setAuthorization = function(request, reply, session, store, user, client, token) {
  if (token.expired()) {
    return Co(function*() {
      ...
      return internals.setCredentials(reply, session, upserted);
    });
  }
