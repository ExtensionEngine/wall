...
module.exports = class extends TokenStore {
  ...
  upsert(tid, data) {
    let _tid = arguments.length === 1 ? tid.tid : tid;
    let _data = arguments.length === 1 ? tid : data;
    const self = this;

    return Co(function*() {
      const validated = yield self.validate(_data);
      const upserted = yield self.collection.findOneAndUpdate({ tid: _tid }, {
